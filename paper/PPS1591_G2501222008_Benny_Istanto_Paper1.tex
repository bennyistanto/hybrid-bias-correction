...
\def\fps@figure{htbp}
\makeatother
% \usepackage{svg}  % not needed; all figures use PNG
...
\title{A Scalable Framework for Enhancing Satellite-Derived Daily Precipitation: Adjusting Values, Aligning Distributions, and Preserving Extremes}
\author{Benny Istanto \\and Rizaldi Boer \\and I Putu Santikayasa}
\date{}
...
\section{2 Conceptual Framework and Theoretical Background}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/fig01_framework.png}}
\caption{Conceptual workflow of the LSEQM+DL hybrid bias correction framework.}
\end{figure}
...
\subsection{3.1 Study Region}\label{study-region}
...
\subsection{3.2 Data and Preprocessing}\label{data-and-preprocessing}
...
\subsection{3.3 Hybrid Bias Correction
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/fig02_workflow.png}}
\caption{Overview of the hybrid bias correction workflow integrating Linear Scaling (LS), Empirical Quantile Mapping (EQM) with Gamma fitting, Generalized Pareto Distribution (GPD) tail adjustment, and Convolutional Neural Network (CNN) refinement.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/fig03_cnn_pipeline.png}}
...
\subsection{3.4 Model Evaluation and Quality
...
\subsubsection{3.4.2 Evaluation Metrics}\label{evaluation-metrics}
...
\subsubsection{3.4.4 Confidence Assessment}\label{confidence-assessment}
...
\subsection{4.1 Progressive Improvement Across Correction
Stages}
...
\subsection{4.2 Spatial Distribution of Correction
Quality}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/fig06_cqi_improvement.png}}
\caption{Figure6}
\end{figure}
% Orphan caption (merge manually): Figure 6. (a) Smoothed CQI difference map showing the mean difference between LSEQM+DL and LS across all 36 dekadal periods (positive values indicate improvement; Gaussian smoothing with \(\sigma = 1.5\) pixels applied for visual clarity). (b) Six-tier quality classification of LSEQM+DL: Excellent (\(\geq\) 0.7, dark green), Good (0.6--0.7, green), Fair-High (0.5--0.6, yellow-green), Fair-Low (0.4--0.5, yellow), Marginal (0.3--0.4, orange), and Poor (\textless{} 0.3, red).
...
\subsection{4.3 Distributional Quality and Extreme Event
Representation}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/fig07_component_boxplots.png}}
\caption{Figure7}
\end{figure}
% Orphan caption (merge manually): Figure 7. Distribution of CQI component scores across all land pixels for LS (blue), LSEQM (orange), and LSEQM+DL (green): (a) Basic Statistical Quality Score (BSQS), (b) Distribution Quality Score (DQS), (c) Temporal Quality Score (TQS), and (d) overall Continuous Quality Index (CQI). Box plots show the median, interquartile range, and 5th--95th percentile whiskers.
...
\subsection{4.4 Cross-Reference
...
\subsection{4.5 Station-Level Ground Truth
...
\subsection{4.6 Role of the Station Density Confidence
Mask}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/fig10_confidence_mask.png}}
\caption{Figure10}
\end{figure}
% Orphan caption (merge manually): Figure 10. (a) Station density confidence mask \(C(x, y)\), ranging from 0 (no station coverage) to 1 (saturated coverage, \(\geq\) 3 stations per \(0.5^{\circ}\) cell). Gaussian smoothing (\(\sigma = 1\) grid cell) prevents sharp boundaries. (b) Relationship between confidence mask value and CQI improvement (LSEQM+DL minus LSEQM): box plots binned by confidence quintile.
...
\subsection{4.7 Temporal Stability of
...
\subsection{4.8 Discussion}\label{discussion}
...
\subsubsection{4.8.4 Limitations}\label{limitations}
...
\subsubsection{4.8.6 Future Directions}\label{future-directions}
