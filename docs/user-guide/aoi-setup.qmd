...
Two of the three items below are sensitivity parameters: `saturation_count` and
`gpd_threshold_percentile` were chosen by inspection against Indonesia, not by formal
optimisation, as was `blend_alpha`. Treat all three as starting points for a new domain.
See [Sensitivity Analysis](../technical/sensitivity-analysis.qmd) for how the outputs move
across their ranges.
