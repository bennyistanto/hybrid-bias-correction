...
`src/bias_correction.py` - LS has no standalone function; it is computed inline at the top of `lseqm()`, before the EQM loop. Driven by the per-dekad call from `notebooks/02_lseqmdl_bias_correction.ipynb`.
