...
`src/deep_learning.py` - the blending step lives inside `apply_deeplearning_model()`, together with the CNN inference. The confidence mask is loaded from `output/station_density/` and passed in by `run_correction_pipeline()` in `src/bias_correction.py`.
