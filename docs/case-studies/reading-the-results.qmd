...
The values shown on the cards are from the full Indonesia run (172 validated stations). They are illustrative of the *shape* of each metric, not the headline result - for that, see the [Indonesia study](indonesia.qmd).
...
Pearson correlation of daily values against the gauge. Higher is better, but for daily satellite precipitation this metric **stays well below 1**, held down by factors the correction cannot touch - see [the correlation ceiling](indonesia.qmd#the-correlation-ceiling).

![Pearson correlation - higher is better, but it stays low for daily fields.](../images/metric-cards/card_corr.png){fig-align="center"}
