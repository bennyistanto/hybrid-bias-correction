...
1. The dekad parameter-fitting strategy assumes enough samples per cell per dekad (~220 daily samples for 22 years; sub-daily would push this up but the *parameter* would be unchanged because the dekad windowing is a property of the meteorology, not the sampling).
...
On the **full-Indonesia run** with 180 BMKG stations, the upper thresholds populate and the curve extends to 150 mm (visible in the manuscript figure). The framework's behaviour is identical; only the sample size changes.
