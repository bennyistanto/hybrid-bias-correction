...
A U-Net or attention-based model would help if we had per-station temporal data densely enough to learn spatial patterns from scratch. We don't. The current CNN is intentionally conservative; replacing it with a heavier architecture is in the [Changelog backlog](changelog.qmd#backlog-next-release) for when training data improves.
...
These three parameters were chosen by inspection rather than formal optimisation. We acknowledge this as a limitation in the paper and in this documentation. A formal sensitivity sweep is on the [Changelog backlog](changelog.qmd#backlog-next-release) for the next release.
