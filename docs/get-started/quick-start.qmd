...
There is a short quickstart notebook for the Bali example at [`notebooks/example_bali/quickstart_bali.ipynb`](https://github.com/bennyistanto/hybrid-bias-correction/blob/main/notebooks/example_bali/quickstart_bali.ipynb). For everything beyond that, the same notebooks that produced the manuscript results are the entry points: only the config you point them at changes.
...
::: {.feature-card}
**Option A - Bali example (recommended for first run).** Uses `config_bali.yml` and the 11 MB Bali bundle that ships in the repository under [data/example_bali/](https://github.com/bennyistanto/hybrid-bias-correction/tree/main/data/example_bali). The nb02 bias-correction step finishes in about 16 minutes on a free Colab CPU; the full nb02 to nb06 chain takes about 72 minutes, most of it in nb03.
:::
