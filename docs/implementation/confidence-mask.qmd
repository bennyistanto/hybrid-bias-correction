...
`load_confidence_mask(confidence_mask_file)` returns the DataArray. `run_correction_pipeline()` passes it to `apply_deeplearning_model()` in `src/deep_learning.py`, which is where the per-pixel `effective_alpha` is computed:
