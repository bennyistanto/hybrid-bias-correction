...
- All-NaN IMERG cells (ocean) are skipped (`continue` at line 217 in `bias_correction.py`).
- Cells where any CPC parameter is NaN return all-NaN (line 529-531 in `distribution_fitting.py`).
- Cells with fewer than 5 wet days return all-zero (line 550-554). This is conservative - the alternative is to propagate IMERG raw, which may be wetter than the climatology supports.
