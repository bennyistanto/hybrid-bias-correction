"""
Bias Correction Script for Daily Precipitation Data - Hybrid LSEQM + DL

This script performs bias correction on daily precipitation data using:
1. **Linear Scaling (LS)**
2. **Empirical Quantile Mapping (EQM)** with Tail Adjustment (Generalized Pareto Distribution, GPD)
3. An optional Deep Learning (DL) model to further refine the bias correction

**Key Features**:
-----------------
1. **Linear Scaling (LS)**: Corrects the mean bias in IMERG data by scaling it with the ratio of CPC to IMERG.
2. **Empirical Quantile Mapping (EQM)**: Matches IMERG’s distribution to CPC’s distribution via gamma-based quantile mapping,
   with GPD tail-fitting for extreme precipitation.
3. **Deep Learning for Bias Correction** (Optional): Trains a convolutional neural network (CNN) on aggregated dekadal data
   to learn a refined correction mapping from IMERG to CPC. The trained model can then be applied to future IMERG data.
4. **Dekad-Based Aggregation**: Aggregates multi-year daily data for each 10-day period (dekad) and processes them as a single unit.
5. **NetCDF Output**: Saves LS-corrected, LSEQM-corrected, and optionally DL-corrected precipitation data in NetCDF format following
   CF conventions.

**Workflow**:
-------------
1. **Load Input Data** (IMERG, CPC, Land-Sea Mask).
2. **Aggregate Data by Dekad**: For each month and each dekad (1–10, 11–20, 21–end), aggregates multi-year data.
3. **Train DL Model** (Optional): If not already trained, train a CNN on the aggregated IMERG vs. CPC data for that dekad.
4. **Bias Correction**:
   - **LS**: Scale IMERG by the ratio of mean CPC to mean IMERG.
   - **EQM**: Adjust the distribution via gamma quantile mapping plus GPD for the upper tail.
   - **DL Correction** (Optional): Apply the trained CNN to further reduce bias on top of EQM results.
5. **Save Outputs**: Writes corrected precipitation as NetCDF files per dekad.

**Usage Instructions**:
-----------------------
1. **Set Directories**: Update the input paths for IMERG, CPC, mask files, etc.
2. **Configure Parameters**: Modify GPD threshold, DL hyperparameters, or cross-validation splits as needed.
3. **Run**: Provide `month` and `dekad` for processing.

**Notes**:
----------
- The GPD tail-fitting is integrated in `gamma_quantile_mapping()`.
- The DL model is optional; if not trained or not provided, the script reverts to LSEQM only.
- Adjust the DL hyperparameters (e.g., filter sizes, dropout rates) based on your dataset size and computational resources.
- All corrected outputs are stored in NetCDF format for compatibility with standard climate-analysis workflows.

**Author**:
Benny Istanto
- GOST/DECSC/DEC Data Group, The World Bank, United States – bistanto@worldbank.org
- Applied Climatology, Bogor Agricultural University, Indonesia – bennyistanto@ipb.ac.id
...
def save_corrected_precip(
    ...
    """
    Output filename follows this convention:
    idn: Indonesia
    ...
    """
    output_file = f'{folder}/idn_cli_{method_abbr}_corrected_imergl_month{month_str}_dekad{dekad_str}.nc4'

    # Check if output file exists
    if os.path.exists(output_file):
        logging.info(f"File {output_file} already exists.")
        set_user_decision()  # Get the user's decision for Skip (S), Overwrite (O), or Abort (A)
...
def lseqm_bias_correction(
    ...
    using data aggregated across years for the same dekad.
    ...
    ----------
    xarray.DataArray
        The bias-corrected precipitation data for the dekad.
    """
...
# The process start from here
def main():
    """
    Main function for dekad-based bias correction with optional DL.

    Steps:
    1. Load multi-year IMERG and CPC datasets.
    2. Prompt user for which month (1-12) and which dekad (1,2,3).
    3. Dynamically handle leap years for the 3rd dekad by scanning the dataset
       to find the maximum day in that month (via get_max_day_in_month()).
    4. Aggregate data across all years for that (month, dekad).
    5. Train (or load) the DL model for that month–dekad combo.
    6. Apply LS + EQM (with GPD tails) to the entire daily data for that month–dekad.
       (Essentially a masked time subset.)
    7. Optionally apply the DL model to produce final correction.
    8. Save the corrected data to NetCDF.
    """

    # Prompt the user for month and dekad
    ...
    logging.info(f"User selected: month={month}, dekad={dekad}")

    # Load IMERG, CPC, and mask
    logging.info("Loading IMERG data from: %s", imerg_file)
    imerg_ds = xr.open_dataset(imerg_file, decode_times=True)
    logging.info("IMERG dataset loaded successfully.")

    logging.info("Loading CPC data from: %s", cpc_file)
    cpc_ds = xr.open_dataset(cpc_file, decode_times=True)
    logging.info("CPC dataset loaded successfully.")

    logging.info("Loading land-sea mask from: %s", mask_file)
    mask_ds = xr.open_dataset(mask_file, decode_times=True)
    land_sea_mask = mask_ds['land']

    # Format month as two-digit string
    month_str = f"{month:02d}"
    # Determine dekad_str based on dekad number, dekad start and end days
    if dekad == 1:
        dekad_str = "01"
        ...
        dekad_str = "21"
        dekad_start, dekad_end = 21, get_max_day_in_month(imerg_ds, month)

    # Align datasets
    cpc_ds, land_sea_mask = reindex_and_align_with_monotonicity(imerg_ds, cpc_ds, land_sea_mask)

    logging.info(f"Using month={month}, dekad={dekad}, day-range={dekad_start}–{dekad_end} ...")

    # Aggregate multi-year data for this month & dekad
    logging.info("Aggregating data across all years for IMERG and CPC...")
    imerg_dekad_data, cpc_dekad_data = aggregate_data_across_years(
        imerg_ds, cpc_ds, month, dekad_start, dekad_end
    )

    # After aggregation
    logging.info(f"IMERG aggregated shape: {imerg_dekad_data.shape}")
    logging.info(f"CPC aggregated shape: {cpc_dekad_data.shape}")

    # Align shapes
    if imerg_dekad_data.shape != cpc_dekad_data.shape:
        logging.warning("Aggregated IMERG and CPC data shapes do not match. Aligning datasets...")
        imerg_dekad_data, cpc_dekad_data = xr.align(imerg_dekad_data, cpc_dekad_data, join='inner')

    # Construct model name and train/load model
    model_name = f"bias_correction_model_month{month_str}_dekad{dekad_str}"
    logging.info(f"Training or loading model: {model_name}")
    model = train_bias_correction_model(imerg_dekad_data, cpc_dekad_data, model_name)

    # Now apply LS + EQM + optional DL for that entire dekad across all years
    logging.info(f"Applying LSEQM correction (and DL if model not None) for month={month}, dekad={dekad_start}-{dekad_end}...")
    corrected_precip = lseqm_bias_correction(
        imerg_ds, cpc_ds, month, dekad_start, dekad_end, model=model,
        month_str=month_str,  # Pass month_str
        dekad_str=dekad_str   # Pass dekad_str
    )

    logging.info("Bias correction for chosen month-dekad completed.")
